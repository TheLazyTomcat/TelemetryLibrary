...
var
  ...
  PacketsBuilder: TTelemetryCommPacketsBuilder = nil;
  Transmitter:    TTelemetryCommClientTransmitter = nil;
...
If not TTelemetryRecipient.SupportsTelemetryAndGameVersionParam(version,params^) then
  ...
  begin
    ...
    PacketsBuilder := TTelemetryCommPacketsBuilder.Create;
    Transmitter := TTelemetryCommClientTransmitter.Create(nil,PacketsBuilder);
  end;
end;

procedure TelemetryLibraryFinal; stdcall;
begin
FreeAndNil(Transmitter);
FreeAndNil(PacketsBuilder);
